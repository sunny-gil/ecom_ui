...
export const apiService = {
  // Products
  getProducts: () => request("/v1/products").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
  getProductById: (id: string) => request(`/v1/products/${id}`).then(item => ({ ...item, id: item._id })),
  ...
  // Blogs
  getBlogs: () => request("/v1/blogs").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
  getBlogById: (id: string) => request(`/v1/blogs/${id}`).then(item => ({ ...item, id: item._id })),
  ...
  // News
  getNews: () => request("/v1/news").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
  getNewsById: (id: string) => request(`/v1/news/${id}`).then(item => ({ ...item, id: item._id })),
  ...
  // Services
  getServices: () => request("/v1/services").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
  ...
  // Testimonials
  getTestimonials: () => request("/v1/testimonials").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
  ...
  // Home Slides
  getHomeSlides: () => request("/v1/home-slides").then(data => data.map((item: any) => ({ ...item, id: item._id }))),
